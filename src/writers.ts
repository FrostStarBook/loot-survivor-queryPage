...
export async function adventurerLeveledUp({ block, tx, event, mysql }: Parameters<CheckpointWriter>[0]) {
  if (!event) return;
  console.log(event.data);
  const owner = toAddress(event.data[0]);
  const adventurer_id = BigInt(event.data[1]);
  ...
  const stat_points_availabl = BigInt(event.data[39]);
  const mutated = BigInt(event.data[40]);
  const previous_level = BigInt(event.data[41]);
  const new_level = BigInt(event.data[42]);
  const timestamp = block.timestamp;
  const blockNumber = block.block_number;

  const adventurerLeveledUp = {
    id: `${owner}/${tx.transaction_hash}`,
    owner: owner,
    adventurer_id: adventurer_id,
    no_message: no_message,
  ...
  // table names are `lowercase(TypeName)s` and can be interacted with sql
  await mysql.queryAsync('INSERT IGNORE INTO adventurerleveledups SET ?', [adventurerLeveledUp]);
}

export async function upgradesAvailable({ block, tx, event, mysql }: Parameters<CheckpointWriter>[0]) {
  if (!event) return;
  console.log(event.data);
  const owner = toAddress(event.data[0]);
  const adventurer_id = BigInt(event.data[1]);
  ...
  // table names are `lowercase(TypeName)s` and can be interacted with sql
  await mysql.queryAsync('INSERT IGNORE INTO upgradesavailables SET ?', [upgradesAvailable]);
}